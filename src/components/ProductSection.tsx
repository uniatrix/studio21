          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-4 mb-16">
              {[
                ...
                <div key={index} className="flex items-center space-x-4 group hover:transform hover:translate-x-2 transition-transform duration-200">
                  <span className="text-3xl text-amber-400 flex-shrink-0 font-bold">→</span>
                  <span className="text-xl text-foreground font-medium">{item}</span>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-16">
              {productTypes.map((product, index) => (
                <div key={index} className="bg-card/50 p-5 rounded-xl shadow-soft border border-border/50 hover:shadow-strong transition-all duration-300">
                  <div className="text-4xl mb-3">{product.icon}</div>
                  <h3 className="text-xl font-semibold text-foreground mb-2">{product.title}</h3>
                  <p className="text-muted-foreground mb-2">{product.description}</p>
                  <div className="text-amber-400 font-bold">{product.count}</div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center space-y-8 mt-16">
              ...
              <div className="space-y-4 text-xl text-muted-foreground">
                <p>This worked for me—and for thousands of others.</p>
                <p className="font-semibold">
                  I'm not special. I just stopped guessing…<br />
